...
{
public:
    virtual float get_call_cost() = 0; //pure virtual function

};
//1st class phone_call
...
public: //methods and functions
	phone_Call(){}  //constructor
	~phone_Call(){} //destructor
	void set_start_time(string n){ start_time = n; } // to set the current time & date of call
	string get_start_time(){ return start_time; } // to get the current time & date of call
	void set_start_date(string n){ date = n; } // to set current date
	string get_start_date(){ return date; } // to get the current date
	void set_call_duration(float n){ call_duration = n; } // to set call duration
	float get_call_duration(){ return call_duration; } // to get call duration
	void set_call_cost(float n){ call_cost = n; } // set the call cost
	float get_call_cost(){ return call_cost; } //to get call cost
	void display_call_details();//display all details of phone call
	void missed_Call_details();// missed call details show
};
// display functions to display call details
...
public:
	phone(){}  //constructor
	~phone(){}  //destructor
	void set_phone_number_one(string n){ phone_number_one = n; } //register the num 1
	string get_phone_number_one(){ return phone_number_one; }   //get the registered num 1
	void set_phone_number_two(string n){ phone_number_two = n; }// register num 1
	string get_phone_number_two(){ return phone_number_two; }   //get the registered num 2
	void set_credit(float n){ credit = n; } //set the credit amount
	float get_credit(){ return credit; }    //get the amount
	void set_history(string n){ call_history = n; } // call history
	string get_history(){ return call_history; }    // get the history
	void set_msg_history(string msg){ message_histoy = msg; } //set receiver's msg history
	string get_msg_history(){ return message_histoy; }         // get the receiver's msg history
	void display_phone_details(); //display the details of call
};
...
public:
	...
	void set_name(string n){ name = n; } //own name set
	string get_name(){ return name; }// get the name
	void set_avg_call_time(float n){ avg_call_time = n; } //set the avg time of call
	float get_avg_call_time(){ return avg_call_time; }  //get the abg time of call
	void set_total_cost(float n){ total_cost = n; }     //cost 20 poisa per sec
	float get_total_cost(){ return total_cost; }        // get the cost
...
// true false input checking for registered number
...
//to add new person to the program using dynamic memory
...
void person_db::add_person(){
	...
	float balance = 0;
	string a;
	cout << "Enter name:"; //own name for registered number
	cin >> n; // taking number from input for registration
	db[index].set_name(n);
    //condition checking using exception handling (try/catch)
	cout << "Enter phone number -\nFor sim1 :";

	//process of taking number for sim 1
	while (1)
	{
		cin >> a;
		{
			...
			catch (const char* msg){ cout << msg << endl; } // print the default msg
		}

	}
	//process of taking number for sim 2
	cout << "For sim2 :";
	while (1){
		cin >> a;
		if (db_is_empty() == true){
...
void person_db::recharge() {
	...
	string num = "";
	cin >> num;
	int i = search_by_phone_number(num); // searching number for recharge (array position)
	if (i == -1){ cout << "number didn't match." << endl; }
	else {
		cout << "Enter amount:";
		float tk = 0;
		cin >> tk; // input balance
		db[i].owned_phone.set_credit(db[i].owned_phone.get_credit() + tk);
	}
}
// checking that number is registered or not
...
float call_charge(float sec){
	return sec * .20; // 20 poisa per sec for call
}
...
void make_a_call(){
	if (database.db_is_empty() == true)cout << "no person exists!\nadd a person first!" << endl;
		...
		string caller_num = ""; // choosing sim for call
		...
		cin >> receiver_num; // taking receiver number as input
...
// for calculate the time these reference are used
		...
		strftime(now_time, 20, "%I:%M:%S%p", &nowinfo); // taking time before making a call
		strftime(now_date, 20, "%x", &nowinfo);  // taking date before the call
		...
		theninfo=*(localtime(&then)); // taking time after ending the call
		float duration = difftime(then, now); // calculate duration in sec
		float cost = call_charge(duration); // calculate the cost using 20 poisa per sec
...
//conditions for missed call
    ...
    //making a call and sending details to the call history
			if (i > -1){
...
void send_a_message(){
	if (database.db_is_empty() == true)cout << "no person exists!\nadd a person first!" << endl;
		...
		cin.ignore(); // taking input of message body
		getline(cin, message);
		...
		cout << "connecting.....\nsending....\nsent!" << endl;
// 150 characters for a single msg
		if (message.length() > 150){
			...
			cout << "cost :.50 BDT" << endl;
			database.db[i].set_total_cost(database.db[i].get_total_cost() + .50);
			database.db[i].owned_phone.set_credit(database.db[i].owned_phone.get_credit() - .50);
		}
...
// sending msg history sent_message
		string sent_message = "";
		sent_message += "\n--------------------------\n";
		sent_message += "To->+0";
		string a = receiver_num;
